...
bool WiFiManager::handleRequest() {
  ...
  if (uri == F("/logout")) {
    _httpAuthUntil = 0;
    server->sendHeader(F("Location"), F("/wm-login"), true);
  ...
  if (uri == F("/wm-login")) {
    ...
    if (server->method() == HTTP_POST) {
      String entered = server->arg(F("password"));
      entered.trim();
      loginOk = entered == _httpAuthPass;
      if (loginOk) {
        _httpAuthClientIp = server->client().remoteIP();
        _httpAuthUntil = millis() + 3600000UL;
    ...
    page += F("<form method='post' action='/wm-login'>");
    page += F("<label>Password</label><input type='password' name='password' maxlength='63' autofocus>");
    page += F("<button type='submit'>Login</button></form>");
    page += FPSTR(HTTP_END);
    HTTPSend(page);
    return false;
  }
  ...
  if (_httpAuthUntil > 0 && millis() <= _httpAuthUntil) {
    sessionValid = server->client().remoteIP() == _httpAuthClientIp;
  }
...
void WiFiManager::handleInfo() {
  ...
  page += F("<h3>PicoWatch</h3><hr><dl>");
  page += F("<dt>Project</dt><dd>PicoWatch</dd>");
...
void WiFiManager::reportStatus(String &page){
  ...
  page += str;
  if(_customStatusHTML != NULL && strlen(_customStatusHTML) > 0){
    page += _customStatusHTML;
  }
}
...
/**
 * set custom status html
 * custom element will be added below wifi status on pages that call reportStatus.
 * @access public
 * @param char element
 */
void WiFiManager::setCustomStatusHTML(const char* html) {
  _customStatusHTML = html;
}
...
/**
 * toggle back button visibility on sub-pages
 * @param boolean enabled
 */
void WiFiManager::setShowBack(boolean enabled){
  _showBack = enabled;
}
...
void WiFiManager::setHttpAuth(const char *user, const char *pass){
  (void)user;
  _httpAuthPass = pass == NULL ? "" : String(pass);
  _httpAuthEnabled = _httpAuthPass.length() >= 8;
  _httpAuthUntil = 0;
  _httpAuthClientIp = IPAddress();
}
